...
//  Tiger
//
//  Created by Eric Seidel on Sun Nov 16 2003.
//  Copyright (c) 2003 Eric Seidel. All rights reserved.
//  A simple Java applet to help explain the "Tiger" hash algorithm
//  Tiger was designed by Ross Anderson, Eli Bihmam:
//  
//
...
public class Tiger extends Applet {
    ...
    private Button Start_Button, Step_Button;
    private Button Show_Button;
	private TextField   ALabel  = new TextField("a = ", 30);
	private TextField   BLabel  = new TextField("b = ", 30);
	private TextField   CLabel  = new TextField("c = ", 30);
	private TextField InputText = new TextField("", 50);
	private List	  StepList	= new List(31, false);
	
	private TextField[] XLabels = new TextField[8];
	
	private int current_state = 0;
	private boolean executingFromButton = false;
	
	private Tiger_Hash hash = new Tiger_Hash();
    
    public TextField DescriptionText = new TextField();

    public TigerFrame MainFrame;
    ...
    public void init()
    {
		...
		MainFrame = new TigerFrame("Tiger Hash Algorithm");
		MainFrame.hide();
    }
    
    private class TigerFrame extends Frame
	{
		public TigerFrame(String Title)
		{
			...
			Label InputTextLabel = new Label("Input: ");
			NorthPanel.add(InputTextLabel);
			NorthPanel.add(InputText);
			
			// setup side list.
			add(StepList, "West");
			
			StepList.add("Save A,B,C");
			StepList.add("Pass 1: Round 1");
			StepList.add("Pass 1: Round 2");
			StepList.add("Pass 1: Round 3");
			StepList.add("Pass 1: Round 4");
			StepList.add("Pass 1: Round 5");
			StepList.add("Pass 1: Round 6");
			StepList.add("Pass 1: Round 7");
			StepList.add("Pass 1: Round 8");
			StepList.add("Key Scheduling");
			StepList.add("Pass 2: Round 9");
			StepList.add("Pass 2: Round 10");
			StepList.add("Pass 2: Round 11");
			StepList.add("Pass 2: Round 12");
			StepList.add("Pass 2: Round 13");
			StepList.add("Pass 2: Round 14");
			StepList.add("Pass 2: Round 15");
			StepList.add("Pass 2: Round 16");
			StepList.add("Key Scheduling");
			StepList.add("Pass 3: Round 17");
			StepList.add("Pass 3: Round 18");
			StepList.add("Pass 3: Round 19");
			StepList.add("Pass 3: Round 20");
			StepList.add("Pass 3: Round 21");
			StepList.add("Pass 3: Round 22");
			StepList.add("Pass 3: Round 23");
			StepList.add("Pass 3: Round 24");
			StepList.add("Feed Forward");
			System.out.println("Finished adding table.");
			//StepList.select(current_state);
			
			StepList.addItemListener(new MainItemListener());
			...
			for(int x = 0; x < 8; x++) {
				XLabels[x] = new TextField("x" + x + " = ", 30);
				XLabels[x].setEditable(false);
				EastPanel.add(XLabels[x]);
			}
            ...
            Start_Button = new Button("Run hashing algorithm");
            ButtonPanel.add(Start_Button);
            Start_Button.addActionListener(new MainButtonListener());
			
			DescriptionText.setText("Enter text to be hashed above, use buttons below to run the algorithm.");
    ...
    private class MainButtonListener implements ActionListener{
        ...
        {	
			if ( ( (Button)e.getSource() ).equals(Show_Button) )
			{
				MainFrame.show();
			} else if ( ( (Button)e.getSource() ).equals(Start_Button) ) {
				// this is a HORRIBLE hack around a bug in java...
				// that and I'm lazy...
				executingFromButton = true;
				// wasteful, but necessary.
				StepList.deselect(0);
				StepList.select(0);
				//runAlgorithmToState(-1);
			} else if ( ( (Button)e.getSource() ).equals(Step_Button) ) {
				current_state++;
				//runAlgorithmToState(current_state);
				StepList.select(current_state);
			}
        }
    }
	
	private void runAlgorithmToState(int theState) {
		byte[] block = new byte[64];
		byte[] bytes = InputText.getText().getBytes();
		long length = bytes.length;
		if (length > 55)
			length = 55;
		
		System.out.println("Got " + length + " bytes: " + printBlock(bytes, 0, (int) length));
		
		for (int x = 0; x < length; x++) {
			block[x] = bytes[x];
		}
		
		System.out.println("Block w/ bytes: " + printBlock(block, 0, 64));
		block[(int)length] = (byte)0x01; // 1000 0000
		System.out.println("Block w/ padding: " + printBlock(block, 0, 64));
		...
		System.out.println("About to hash: " + printBlock(block, 0, 64));
		hash.coreReset();
		
		if (theState != 0)
			hash.coreUpdate(block, 0, theState);
		
		for(int x = 0; x < 8; x++) {
			XLabels[x].setText("x" + x + " = " + printBlock(block, 8 * x, 8));
		}
		
		hash.coreDigest(block, 0);
		System.out.println("Full result: " + printBlock(block, 0, 64));
		
		ALabel.setText("a = " + printBlock(block, 0, 8) );
		BLabel.setText("b = " + printBlock(block, 8, 8) );
		CLabel.setText("c = " + printBlock(block, 16, 8));
		
		if (theState == -1)
			DescriptionText.setText("Algorithm run (final result in a, b, c) and reset, ready for the next entry!");
		else
			displayComments(theState);
	}

	private String roundArgs(int round) {
		if ((round % 3) == 0)
			return "a, b, c";
		else if ((round % 3) == 1)
			return "b, c, a";
	...
	private void displayComments(int theState) {
		
		if (theState == 0) {
			DescriptionText.setText("First we set A, B, C to their pre-defined salt values.");
		} else if (theState < 9) {
			int round = theState;
			DescriptionText.setText("Round " + round + ", we apply round(" + roundArgs(round) + ", x" + (round-1) + ", 5)");
		} else if (theState == 9) {
			DescriptionText.setText("We apply the first Key Schedule");
		} else if (theState < 18) {
			int round = theState - 1;
			DescriptionText.setText("Round " + round + ", we apply round(" + roundArgs(round) + ", x" + (round-1) + ", 7)");
		} else if (theState == 18) {
			DescriptionText.setText("We apply the Key Schedule a second time.");
		} else if (theState < 27) {
			int round = theState - 2;
			DescriptionText.setText("Round " + round + ", we apply round(" + roundArgs(round) + ", x" + (round-1) + ", 9)");
		} else if (theState == 27) {
			DescriptionText.setText("Finally, we feed forward A, B, C (for the next block), and we're done!");
		} else {
			DescriptionText.setText("Click Run Alogirithm to reset and try another!");
		}
	}
	...
	private class MainItemListener implements ItemListener
    {
		public void itemStateChanged(ItemEvent e)
		{
			if (e.getStateChange() == ItemEvent.SELECTED) {
				current_state = StepList.getSelectedIndex();
				if (executingFromButton) {
					runAlgorithmToState(-1);
					current_state = 0;
					executingFromButton = false;
				} else
					runAlgorithmToState(current_state);
			}
		}
	...
	// Code derived from snippet at http://java.about.com/library/snippets/bl-hex-to-string.htm --
